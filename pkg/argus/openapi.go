...
func NewOpenAPI(ctx context.Context, instrumentation instrumentation.Instrumentation) (*OpenAPI, error) {
	apiserver, err := argusapiserver.NewFactory(
	...
	reflector.Spec.WithInfo(*(&openapi3.Info{}).
		...
		WithTermsOfService("https://argus.example.com/terms-of-service/").
		WithContact(*(&openapi3.Contact{}).
			WithName("Argus Support").
			WithURL("https://argus.example.com").
			WithEmail("support@argus.example.com")),
	)

	reflector.Spec.WithServers(
		// Default server
		*(&openapi3.Server{}).WithURL("https://{host}:{port}{base_path}").
			WithDescription("The fully qualified URL to the Argus APIServer.").
			WithVariablesItem("host", *(&openapi3.ServerVariable{}).
				WithDefault("localhost").
				WithDescription("The host of the Argus APIServer")).
			WithVariablesItem("port", *(&openapi3.ServerVariable{}).
				WithDefault("8080").
				WithDescription("The port of the Argus APIServer")).
			WithVariablesItem("base_path", *(&openapi3.ServerVariable{}).
				WithDefault("/").
				WithDescription("The base path of the Argus APIServer")),
	)

	reflector.SpecSchema().SetAPIKeySecurity(authtypes.IdentNProviderAPIKey.StringValue(), "Argus-Api-Key", openapi.InHeader, "API Keys")
