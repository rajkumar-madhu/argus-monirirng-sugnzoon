...
import (
	...
	"github.com/your-org/argus/pkg/alertmanager/nfmanager/nfroutingstore/sqlroutingstore"
	...
	"github.com/your-org/argus/pkg/authn/authnstore/sqlauthnstore"
	...
	pkgtokenizer "github.com/your-org/argus/pkg/tokenizer"
...
// telemetry metadata store (reused elsewhere in argus.New), the per-signal
...
func New(
	...
	instrumentation.Logger().InfoContext(ctx, "starting Argus", slog.String("version", version.Info.Version()), slog.String("variant", version.Info.Variant()), slog.String("commit", version.Info.Hash()), slog.String("branch", version.Info.Branch()), slog.String("go", version.Info.GoVersion()), slog.String("time", version.Info.Time()))
	instrumentation.Logger().DebugContext(ctx, "loaded Argus config", slog.Any("config", config))
	...
	rulerInstance, err := factory.NewProviderFromNamedMap(ctx, providerSettings, config.Ruler, rulerProviderFactories(cache, alertmanager, sqlstore, telemetrystore, telemetryMetadataStore, prometheus, orgGetter, modules.RuleStateHistory, querier, queryParser), "argus")
	if err != nil {
		return nil, err
	}
