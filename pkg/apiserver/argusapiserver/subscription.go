...
func (provider *provider) addSubscriptionRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v1/subscriptions", handler.New(provider.authzMiddleware.CheckResources(provider.subscriptionHandler.Create, authtypes.ArgusAdminRoleName), handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v1/subscriptions", handler.New(provider.authzMiddleware.CheckResources(provider.subscriptionHandler.Update, authtypes.ArgusAdminRoleName), handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v1/subscriptions", handler.New(provider.authzMiddleware.CheckResources(provider.subscriptionHandler.Get, authtypes.ArgusAdminRoleName), handler.OpenAPIDef{
		ID:                  "GetSubscription",
		Tags:                []string{"subscriptions"},
