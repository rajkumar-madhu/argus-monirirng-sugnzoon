...
func (provider *provider) addAuthDomainRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v2/auth_domains", handler.New(
		provider.authzMiddleware.CheckResources(provider.authDomainHandler.List, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "ListAuthDomains",
	...
	if err := router.Handle("/api/v2/auth_domains", handler.New(
		provider.authzMiddleware.CheckResources(provider.authDomainHandler.Create, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "CreateAuthDomain",
			Tags:                []string{"authdomains"},
	...
	if err := router.Handle("/api/v2/auth_domains/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.authDomainHandler.Get, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "GetAuthDomain",
			Tags:                []string{"authdomains"},
	...
	if err := router.Handle("/api/v2/auth_domains/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.authDomainHandler.Update, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "UpdateAuthDomain",
			Tags:                []string{"authdomains"},
	...
	if err := router.Handle("/api/v2/auth_domains/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.authDomainHandler.Delete, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "DeleteAuthDomain",
			Tags:                []string{"authdomains"},
...
func authDomainEffectiveRoleNames(roleMapping *authtypes.RoleMapping) []string {
	if roleMapping == nil {
		return []string{authtypes.ArgusViewerRoleName}
	}
