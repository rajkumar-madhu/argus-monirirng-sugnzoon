...
func (provider *provider) addDashboardRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v2/dashboards", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.ListV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "ListDashboardsV2",
	...
	if err := router.Handle("/api/v2/users/me/dashboards", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.ListForUserV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboards", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.CreateV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboards/{id}/clone", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.CloneV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "CloneDashboardV2",
			Tags:                []string{"dashboard"},
	...
	if err := router.Handle("/api/v2/dashboards/{id}/migrate", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.MigrateV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboards/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.GetV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "GetDashboardV2",
			Tags:                []string{"dashboard"},
	...
	if err := router.Handle("/api/v2/dashboards/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.UpdateV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboards/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.PatchV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:          "PatchDashboardV2",
			Tags:        []string{"dashboard"},
	...
	if err := router.Handle("/api/v2/dashboards/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.DeleteV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboards/{id}/lock", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.LockV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "LockDashboardV2",
			Tags:                []string{"dashboard"},
	...
	if err := router.Handle("/api/v2/dashboards/{id}/lock", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.UnlockV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboards/system/{name}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.GetSystemDashboard, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "GetSystemDashboard",
			Tags:                []string{"dashboard"},
			Summary:             "Get system dashboard",
			Description:         "Returns a dashboard Argus ships and owns, addressed by its stable definition name (e.g. `ai-o11y-overview`) rather than its id. System dashboards are read-only and upgraded through releases. The dashboard's own `name` field carries a reserved prefix that the path segment must not include.",
			Request:             nil,
			RequestContentType:  "",
	...
	if err := router.Handle("/api/v2/users/me/dashboards/{id}/pins", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.PinV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "PinDashboardV2",
			Tags:                []string{"dashboard"},
	...
	if err := router.Handle("/api/v2/users/me/dashboards/{id}/pins", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.UnpinV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "UnpinDashboardV2",
			Tags:                []string{"dashboard"},
	...
	if err := router.Handle("/api/v2/dashboard_views", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.ListViews, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboard_views", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.CreateView, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboard_views/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.UpdateView, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/dashboard_views/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.DeleteView, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v1/dashboards/{id}/public", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.CreatePublic, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v1/dashboards/{id}/public", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.GetPublic, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v1/dashboards/{id}/public", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.UpdatePublic, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v1/dashboards/{id}/public", handler.New(
		provider.authzMiddleware.CheckResources(provider.dashboardHandler.DeletePublic, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
