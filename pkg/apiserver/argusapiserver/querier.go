...
func (provider *provider) addQuerierRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v5/query_range", handler.New(provider.authzMiddleware.CheckResources(provider.querierHandler.QueryRange, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName), handler.OpenAPIDef{
		ID:                 "QueryRangeV5",
		Tags:               []string{"querier"},
		Summary:            "Query range",
	...
	if err := router.Handle("/api/v5/query_range/preview", handler.New(provider.authzMiddleware.CheckResources(provider.querierHandler.QueryRangePreview, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName), handler.OpenAPIDef{
