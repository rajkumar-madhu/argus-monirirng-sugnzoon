...
import (
	...
	"github.com/your-org/argus/pkg/types/gatewaytypes"
...
func (provider *provider) addGatewayRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v2/gateway/ingestion_keys", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.GetIngestionKeys, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "GetIngestionKeys",
			Tags:                []string{"gateway"},
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/search", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.SearchIngestionKeys, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.CreateIngestionKey, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/{keyId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.UpdateIngestionKey, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/{keyId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.DeleteIngestionKey, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/{keyId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.GetIngestionKey, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "GetIngestionKey",
			Tags:                []string{"gateway"},
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/{keyId}/limits", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.GetIngestionKeyLimits, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/{keyId}/limits", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.DeprecatedCreateIngestionKeyLimit, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/limits/{limitId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.UpdateIngestionKeyLimit, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_keys/limits/{limitId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.DeleteIngestionKeyLimit, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_limits", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.CreateIngestionKeyLimit, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
	...
	if err := router.Handle("/api/v2/gateway/ingestion_limits/{limitId}", handler.New(
		provider.authzMiddleware.CheckResources(provider.gatewayHandler.GetIngestionKeyLimit, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
