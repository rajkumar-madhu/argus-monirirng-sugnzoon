...
func (provider *provider) addSavedViewRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v2/saved_views", handler.New(
		provider.authzMiddleware.CheckResources(provider.savedViewHandler.ListV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "ListSavedViews",
	...
	if err := router.Handle("/api/v2/saved_views", handler.New(
		provider.authzMiddleware.CheckResources(provider.savedViewHandler.CreateV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "CreateSavedView",
			Tags:                []string{"saved_view"},
	...
	if err := router.Handle("/api/v2/saved_views/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.savedViewHandler.GetV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "GetSavedView",
			Tags:                []string{"saved_view"},
	...
	if err := router.Handle("/api/v2/saved_views/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.savedViewHandler.UpdateV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "UpdateSavedView",
			Tags:                []string{"saved_view"},
	...
	if err := router.Handle("/api/v2/saved_views/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.savedViewHandler.Delete, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName),
		handler.OpenAPIDef{
			ID:                  "DeleteSavedView",
			Tags:                []string{"saved_view"},
