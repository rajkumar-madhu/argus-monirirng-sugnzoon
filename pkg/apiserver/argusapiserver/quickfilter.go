...
func (provider *provider) addQuickFilterRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v2/quick_filters", handler.New(
		provider.authzMiddleware.CheckResources(provider.quickFilterHandler.ListQuickFiltersV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "ListQuickFilters",
	...
	if err := router.Handle("/api/v2/quick_filters/{source}", handler.New(
		provider.authzMiddleware.CheckResources(provider.quickFilterHandler.GetQuickFiltersV2, authtypes.ArgusAdminRoleName, authtypes.ArgusEditorRoleName, authtypes.ArgusViewerRoleName),
		handler.OpenAPIDef{
			ID:                  "GetQuickFilters",
			Tags:                []string{"quick_filter"},
	...
	if err := router.Handle("/api/v2/quick_filters/{source}", handler.New(
		provider.authzMiddleware.CheckResources(provider.quickFilterHandler.UpdateQuickFiltersV2, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
