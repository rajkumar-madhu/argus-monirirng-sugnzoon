...
func (provider *provider) addRoleRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v1/roles", handler.New(
		provider.authzMiddleware.CheckResources(provider.authzHandler.Create, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "CreateRole",
			Tags:                []string{"role"},
			Summary:             "Create role",
	...
	if err := router.Handle("/api/v1/roles", handler.New(
		provider.authzMiddleware.CheckResources(provider.authzHandler.List, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "ListRoles",
			Tags:                []string{"role"},
			Summary:             "List roles",
	...
	if err := router.Handle("/api/v1/roles/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.authzHandler.Get, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "GetRole",
			Tags:                []string{"role"},
			Summary:             "Get role",
	...
	if err := router.Handle("/api/v1/roles/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.authzHandler.Update, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "UpdateRole",
			Tags:                []string{"role"},
			Summary:             "Update role",
	...
	if err := router.Handle("/api/v1/roles/{id}", handler.New(
		provider.authzMiddleware.CheckResources(provider.authzHandler.Delete, authtypes.ArgusAdminRoleName),
		handler.OpenAPIDef{
			ID:                  "DeleteRole",
			Tags:                []string{"role"},
			Summary:             "Delete role",
