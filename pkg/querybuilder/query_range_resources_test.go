...
func TestQueryRangeResources(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			body: builderQueryBody("logs", "argus.workspace.key.id = 'checkout' AND status = 500"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/checkout"},
			},
		},
		{
			name: "resource prefixed key",
			body: builderQueryBody("traces", "resource.argus.workspace.key.id = 'checkout'"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceTraces, ID: "builder_query/argus.workspace.key.id/checkout"},
			},
		},
		{
			name: "in atom requires every value",
			body: builderQueryBody("logs", "argus.workspace.key.id IN ('b', 'a')"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/a"},
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/b"},
			},
		},
		{
			name: "multiple equality atoms each require a grant",
			body: builderQueryBody("logs", "argus.workspace.key.id = 'b' AND argus.workspace.key.id = 'a'"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/a"},
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/b"},
			},
		...
		},
		{
			...
			body: builderQueryBody("logs", "argus.workspace.key.id = 'a' OR status = 500"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/*"},
			},
		},
		{
			...
			body: builderQueryBody("logs", "NOT argus.workspace.key.id = 'a'"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/*"},
			},
		},
		{
			...
			body: builderQueryBody("logs", "argus.workspace.key.id != 'a'"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/*"},
			},
		},
		{
			...
			body: builderQueryBody("logs", "argus.workspace.key.id = 'check out/2'"),
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/check out/2"},
			},
		},
		{
			name: "audit source maps to audit logs resource",
			body: `{"compositeQuery":{"queries":[{"type":"builder_query","spec":{"signal":"logs","source":"audit","filter":{"expression":"argus.workspace.key.id = 'a'"}}}]}}`,
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceAuditLogs, ID: "builder_query/argus.workspace.key.id/a"},
			},
		},
		{
			name: "ai builder query maps to traces resource without a signal",
			body: `{"compositeQuery":{"queries":[{"type":"builder_ai_query","spec":{"filter":{"expression":"argus.workspace.key.id = 'checkout'"}}}]}}`,
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceTraces, ID: "builder_ai_query/argus.workspace.key.id/checkout"},
			},
		},
		{
		...
		},
		{
			...
			body: `{"compositeQuery":{"queries":[{"type":"builder_query","spec":{"name":"A","signal":"traces","disabled":true,"filter":{"expression":"argus.workspace.key.id = 'checkout'"}}},{"type":"builder_query","spec":{"name":"B","signal":"traces","disabled":true,"filter":{"expression":"argus.workspace.key.id = 'checkout' AND has_error = true"}}},{"type":"builder_trace_operator","spec":{"name":"T1","expression":"A => B","returnSpansFrom":"A"}}]}}`,
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceTraces, ID: "builder_query/argus.workspace.key.id/checkout"},
			},
		},
		{
			...
			body: `{"variables":{"key":{"value":"checkout"}},"compositeQuery":{"queries":[{"type":"builder_query","spec":{"signal":"logs","filter":{"expression":"argus.workspace.key.id = $key"}}}]}}`,
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/checkout"},
			},
		},
		{
			...
			body: `{"compositeQuery":{"queries":[{"type":"builder_query","spec":{"signal":"logs","filter":{"expression":"argus.workspace.key.id = 'a'"}}},{"type":"builder_query","spec":{"signal":"logs","filter":{"expression":"argus.workspace.key.id='a'"}}}]}}`,
			expected: []coretypes.ResourceWithID{
				{Resource: coretypes.ResourceTelemetryResourceLogs, ID: "builder_query/argus.workspace.key.id/a"},
			},
		},
...
func TestQueryRangeResourcesErrors(t *testing.T) {
	bodies := []string{
		...
		`{}`,
		builderQueryBody("logs", "argus.workspace.key.id = "),
...
func TestTelemetrySelector(t *testing.T) {
	...
	assert.Equal(t, []string{"builder_query/argus.workspace.key.id/a", "builder_query/argus.workspace.key.id/*", "builder_query/*", "*"}, selectorValues("builder_query/argus.workspace.key.id/a"))
	...
	assert.Equal(t, []string{"builder_query/argus.workspace.key.id/a/b", "builder_query/argus.workspace.key.id/*", "builder_query/*", "*"}, selectorValues("builder_query/argus.workspace.key.id/a/b"))
}
