...
func NewFactory(
	...
	return factory.NewProviderFactory(factory.MustNewName("argus"), func(ctx context.Context, settings factory.ProviderSettings, config alertmanager.Config) (alertmanager.Alertmanager, error) {
...
func New(
	...
	settings := factory.NewScopedProviderSettings(providerSettings, "github.com/your-org/argus/pkg/alertmanager/argusalertmanager")
...
func (provider *provider) Start(ctx context.Context) error {
	...
	ticker := time.NewTicker(provider.config.Argus.PollInterval)
	defer ticker.Stop()
	for {
		select {
		case <-provider.stopC:
...
func (provider *provider) UpdateChannelByReceiverAndID(ctx context.Context, orgID string, receiver *alertmanagertypes.Receiver, id valuer.UUID) error {
	...
	if err := config.SetGlobalConfig(provider.config.Argus.Global); err != nil {
		return err
	}
...
func (provider *provider) Config() alertmanagerserver.Config {
	return provider.config.Argus.Config
}
...
func (provider *provider) SetDefaultConfig(ctx context.Context, orgID string) error {
	config, err := alertmanagertypes.NewDefaultConfig(provider.config.Argus.Global, provider.config.Argus.Route, orgID)
	if err != nil {
		return err
	}
