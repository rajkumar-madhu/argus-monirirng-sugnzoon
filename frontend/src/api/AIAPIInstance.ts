...
/** Header that tells the AI backend which Argus instance to query against. */
export const SIGNOZ_URL_HEADER = 'X-Argus-URL';

/**
 * Sets `X-Argus-URL` on every outgoing AI Assistant request. The backend
 * needs the originating Argus instance URL for multi-tenant deployments;
 * when omitted it falls back to its `SIGNOZ_API_URL` env var.
 */
export const interceptorsRequestArgusUrl = (
	value: InternalAxiosRequestConfig,
): InternalAxiosRequestConfig => {
	if (value.headers) {
		value.headers[SIGNOZ_URL_HEADER] = getArgusInstanceUrl();
	}
	return value;
};
 ...
 *
 * Mirrors the request/response interceptor stack of the main Argus axios
...
AIAssistantInstance.interceptors.request.use(interceptorsRequestArgusUrl);
